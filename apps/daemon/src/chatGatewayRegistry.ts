...
export type ChatGatewayFactory = (id: string, config: ChatGatewayConfig) => ChatGatewayProvider;
...
export class ChatGatewayRegistry {
  ...
  constructor(options: ChatGatewayRegistryOptions = {}) {
    ...
    for (const [id, gatewayConfig] of Object.entries(options.config?.chat?.gateways ?? {})) {
    ...
    const configuredRoutes = Object.values(options.config?.chat?.routes ?? {}).map(toRuntimeRoute);
...
export function defaultChatGatewayFactory(env: NodeJS.ProcessEnv = process.env): ChatGatewayFactory {
  ...
  switch (config.type) {
    case 'discord': {
      ...
      return new DiscordChatGatewayProvider({
        ...
        ...(config.webhookName !== undefined ? { webhookName: config.webhookName } : {}),
        ...(config.webhookAvatarUrl !== undefined ? { webhookAvatarUrl: config.webhookAvatarUrl } : {}),
        ...(config.ignoreOwnMessages !== undefined ? { ignoreOwnMessages: config.ignoreOwnMessages } : {}),
        ...(config.ignoreBotMessages !== undefined ? { ignoreBotMessages: config.ignoreBotMessages } : {})
      });
    }
  }
}
...
function toRuntimeRoute(route: ChatGatewayRouteConfig): ChatGatewayRoute {
  return {
    ...
    ...(route.conversationKind !== undefined ? { conversationKind: route.conversationKind } : {}),
