...
For Herdr configs generated by `agent-room init --room my-project --runtime herdr`, the Herdr session is `agentroom`, the Herdr CLI command is `herdr`, and the Herdr workspace is `my-project`. Override the Herdr session with `agent-room init --runtime-session <name>`, and override the Herdr command with `agent-room init --runtime-cli <command>`.
...
```bash
herdr --session agentroom
# or, when using a local dev binary:
herdr-dev --session agentroom
```
...
herdr --session agentroom status
```
