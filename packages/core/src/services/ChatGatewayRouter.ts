...
export interface ChatGatewayRouterOptions {
  ...
  runtimeProviderForBinding?: (binding: RuntimeBinding) => RuntimeProvider | Promise<RuntimeProvider>;
}
...
export class ChatGatewayRouter {
  ...
  async handleInbound(message: ChatInboundMessage): Promise<ChatGatewayRouteResult> {
  ...
  private findRoute(message: ChatInboundMessage): ChatGatewayRoute | undefined {
    return this.routes.find(
      (route) =>
        ...
        (route.threadId === undefined || route.threadId === message.conversation.threadId)
    );
  }

  private async sendToAgentStdin(agentId: string, message: ChatInboundMessage): Promise<void> {
    if (!this.runtimeProviderForBinding) {
      throw new Error('agent-stdin chat routes require runtimeProviderForBinding');
    }

    const binding = await this.service.getRuntimeBinding(agentId);
    if (!binding) throw new Error(`No runtime binding found for chat route target '${agentId}'`);
...
function connectorActor(message: ChatInboundMessage): ActorRef {
  return {
    ...
    ...(message.sender.displayName !== undefined ? { displayName: message.sender.displayName } : {})
  };
}
